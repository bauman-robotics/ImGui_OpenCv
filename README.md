...
- Индикация загрузки процессора окном приложения
- Автоматическое восстановление соединения через socket при потери связи.
- Возможность приема массива данных в ASCII кодировке для визуализации в одном пакете (Com-порт и Socket) 
- Возможность приема бинарных пакетов с данными типа int16 (до 700 значений в пакете при паузах между пакетами 10 ms) через wifi (1.1 МБит/сек.)
- Логирование принятых пакетов (бинарных и в ASCII кодах) полученных через wifi. 
- Управление потоком передачи через wi-fi соединение. Отправка команд клиенту - форма генерируемого сигнала, паузы между пакетами, число данных в одном пакете.
## Конфигурация

Проект использует файл конфигурации `imgui.ini`, который создаётся при первом запуске приложения. В этом файле запоминается положение окна перед закрытием приложения, а также можно изменить префикс принимаемых ASCII данных.
...
### Режим 2: Визуализация у-координаты движения мышки

![Режим 2](images/Pic_3.png)

### Режим 2: Визуализация данных полученных через web-socket

![Режим 2](images/Screenshot_4.png)
