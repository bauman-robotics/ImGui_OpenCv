...
baud_rate=115200 (не влиеят, если мы используем виртуальный com-порт)
...
- **baud_rate**: Скорость передачи данных (например, `115200`) (Не влиеят, если мы используем виртуальный com-порт)
...
Данные по COM-порту передаются в ASCII кодах. Убедитесь, что микроконтроллер отправляет данные в нужном формате. Пример валидных данных:
